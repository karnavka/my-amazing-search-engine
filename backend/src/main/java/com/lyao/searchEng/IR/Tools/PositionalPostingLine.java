...
// class for comparing lines during merging blocks and keeping track of which
    ...
    public class PositionalPostingLine implements LineForMerging {
        String term;
        ArrayList<TempDictItem> postings;
        int blockIndex;
        public PositionalPostingLine(String line, int blockIndex) {
            ...
            if (parts.length > 1 && !parts[1].isBlank()) {
                ...
                for (String postingToken : postingTokens) {
                    String[] positions = postingToken.split(",");
                    int docID = Integer.parseInt(positions[0]);
                    float position = Float.parseFloat(positions[1]);
                    postings.add(new PositionalPostingItem(docID, position));
                }
            }
